...
class Functions_Practice < MiniTest::Test
  ...
  def test_add_string_as_number()
    string_as_int = add_string_as_number( "1", "2" ).to_i
    assert_equal( 3, string_num )
  end
  ...
  def test_volume_of_cube()
      new_cube = volume(4)
      assert_equal(16, new_cube)
end
#   end

  Given the radius of a sphere calculate the volume
  def test_volume_of_sphere()
      sphere_volume = vol(66).to_f
...
#   end
  ...
  def test_fahrenheit_to_celsius()
      celsius = fahrenheit_to_celcius(222).to_f
      assert_equal(6.9375, celsius)
end

def test_area_of_right_angled_triangle()
    area = right_angled_triangle(25, 98)
    assert_equal(1225, area)
end
